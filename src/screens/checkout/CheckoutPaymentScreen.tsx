...
export function CheckoutPaymentScreen() {
  ...
  const { selectedAddress, selectedDelivery, appliedCoupon, selectedPaymentMethodId, setSelectedPaymentMethodId } = useCheckout();
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      clearCart();
      navigation.reset({ index: 0, routes: [{ name: 'OrderConfirmation', params: { orderId: newOrder.id } }] });
    } finally {
      setPlacing(false);
    }
  };
    ...
    <SafeAreaView edges={['bottom']} style={styles.screen}>
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
              ...
              <View key={method.id}>
                ...
                {selected && method.type === 'card' && (
                  <View style={styles.cardForm}>
                    ...
                    <Text style={styles.mockNote}>This is a demo checkout — no real payment is processed.</Text>
                  </View>
                )}
              </View>
            );
        ...
        <View style={styles.summaryCard}>
          ...
          <SummaryRow label={`Subtotal (${enrichedItems.length} items)`} value={formatPrice(subtotal)} />
          {couponDiscount > 0 && <SummaryRow label="Coupon Discount" value={`- ${formatPrice(couponDiscount)}`} valueColor={colors.success} />}
          <SummaryRow label="Delivery Fee" value={deliveryFee === 0 ? 'FREE' : formatPrice(deliveryFee)} valueColor={deliveryFee === 0 ? colors.success : undefined} />
